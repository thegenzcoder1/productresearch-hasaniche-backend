...
router.post('/products/:id/ad-links', (req, res) => {
  const { url, label, impression, days_old, ad_type } = req.body || {};
  if (!url || !url.trim()) return res.status(400).json({ error: 'URL required' });
  const info = db.prepare(
    `INSERT INTO ad_links (product_id, url, label, impression, days_old, ad_type, status) VALUES (?, ?, ?, ?, ?, ?, 'pending')`
  ).run(req.params.id, url.trim(), label ?? null, impression == null || impression === '' ? null : Number(impression), days_old ?? null, ad_type ?? null);
  const row = db.prepare(`SELECT * FROM ad_links WHERE id=?`).get(info.lastInsertRowid);
  res.status(201).json(row);
});

router.patch('/ad-links/:id', (req, res) => {
  const fields = ['url', 'label', 'impression', 'days_old', 'ad_type', 'status'].filter((k) => k in (req.body || {}));
